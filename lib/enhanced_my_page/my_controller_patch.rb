  ...
  module MyControllerPatch
		...
		module InstanceMethods
			...
			def add_block_with_enhanced_my_page
			    ...
			    #&& Query.count(:all, :conditions => ["project_id IS NULL AND is_public=? AND id=?", true, ])>0
			    ...
			    if block.include?('query')
			    	render :partial => "query_block", :locals => {:user => @user, :block_name => 'query', :query_id => block.sub(/^(query)/,'')}
			    else
